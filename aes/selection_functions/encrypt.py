...
def _sub_bytes_2(data, guesses, key):
    A = aes.add_round_key(data, key)
    B = aes.sub_bytes(A)
    C = aes.shift_rows(B)
    D = aes.mix_columns(C)                          
    E = _add_round_key(data=D, guesses=guesses)
    F = aes.sub_bytes(E)
    return F
...
class SecondSubBytes:
    """Build an attack selection function which computes intermediate values after AES encrypt sub bytes (S-box) operation at second round, for guesses values.
